...
export default class TextProcessor<M, S> {
    ...
    process(model: M, state: S, functions: {[name: string]: Function} = {}, rescan: boolean = true): string {
        if (this.source === "") {
            return "";
        }
        if (rescan) {
        ...
        for (let i = this.textTokens.length - 1; i >= 0; i--) {
            ...
            if (!!token.ast) {
                ...
                res = res.substring(0, token.position) + value + res.substring(token.position + token.lexeme.length);
            }
        }
        return res;
    }
    ...
    private generateAST(text: string, offset: number, type: TextTokenType): Expression.AST {
