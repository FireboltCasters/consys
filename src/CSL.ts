...
export default class CSL {
    ...
    private static isString(string: string): boolean {
        return string.startsWith(Symbols.STRING_SYMBOL) && string.endsWith(Symbols.STRING_SYMBOL);
    }
    ...
    isCharWithinString(srcString: string, charIndex: number): boolean {
        if (charIndex === 0 || charIndex === srcString.length - 1 || srcString.charAt(charIndex) === Symbols.STRING_SYMBOL) {
            return false;
        }
    ...
    isCharWithinFunction(srcString: string, charIndex: number): boolean {
        ...
        if (charIndex === 0 || charIndex === srcString.length - 1 || char === "(" || char === ")") {
            return false;
        }
        ...
        for (let i = 0; i < openingBracketIndices.length; i++) {
            ...
            if (leftCharOfOpeningBracket.match(/\w/g)) {
                ...
                if (charIndex > openingBracketIndex && charIndex < closingBracketIndex) {
                    return true;
                }
            }
        }
    ...
    getMessageTokens(srcString: string): string[] {
        ...
        for (let i = 0; i < srcString.length - 1; i++) {
            ...
            if (char.match(/\s/g) && nextChar.match(/^(?!.*\s).*$/g) && !this.isCharWithinFunction(srcString, i)) {
                tokenStart = i + 1;
            }
    ...
    getSplitTokens(srcString: string): string[] {
        ...
        for (let i = 0; i < srcString.length; i++) {
            let char = srcString.charAt(i);
            if (char === Symbols.COND_SEPARATOR && !this.isCharWithinString(srcString, i)) {
                splitIndex = i;
                break;
            }
        }
    ...
    private generateConditionalString(data: string): string {
        ...
        while (!done) {
            ...
            if (startChar === Symbols.MODEL_PREFIX || startChar === Symbols.STATE_PREFIX) {
            ...
            // we have some sort of statement here, we must do this by hand unfortunately
                ...
                throw Error("Unable to parse statement: " + data + ", char: " + startChar + ", index: " + startIndex);
            }
            ...
            if (iterations++ > 1000) {
                throw Error("Maximum number of parsing iterations reached, there is a syntax error here: " + data);
            }
        }
    ...
    private isStatementToken(token: string): boolean {
        return this.isFunctionToken(token) && !token.includes("(") && !token.includes(")");
    }
    ...
    private getSymbolForToken(token: string): string {
        ...
        if (CSL.isModelValue(token)) {
            ...
            // map to ts syntax
            switch (token) {
                case Symbols.LESS:          return "<";
                case Symbols.LESS_EQUAL:    return "<=";
                case Symbols.EQUAL:         return "==";
                case Symbols.NOT_EQUAL:     return "!=";
                case Symbols.GREATER_EQUAL: return ">=";
                case Symbols.GREATER:       return ">";
                case Symbols.PLUS:          return "+";
                case Symbols.MINUS:         return "-";
                case Symbols.TIMES:         return "*";
                case Symbols.DIV:           return "/";
                case Symbols.MOD:           return "%";
                case Symbols.BRACKET_OPEN:  return "(";
                case Symbols.BRACKET_CLOSE: return ")";
                case Symbols.AND:           return "&&";
                case Symbols.OR:            return "||";
                default:                    return "";
            }
        }
    }

    getMessage<M, S>(msgString: string, model: M, state: S, functions: { [key: string]: Function }): string {
        ...
        for (let i = 0; i < modelKeys.length; i++) {
            ...
            message = message.replace(modelKeyWithPrefix, CSL.getObjectValue(model, modelKey));
        }

        for (let i = 0; i < stateKeys.length; i++) {
            let stateKey = stateKeys[i];
            let stateKeyWithPrefix = Symbols.STATE_PREFIX + stateKey;
            message = message.replace(stateKeyWithPrefix, CSL.getObjectValue(state, stateKey));
        }
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (this.isStatementToken(token)) {
                ...
                let functionValue = this.evaluateFunction(token, model, state, functions);
    ...
    private evaluateFunction<M, S>(token: string, model: M, state: S, functions: { [key: string]: Function }): any {
        ...
        for (let i = 0; i < argsStrings.length; i++) {
            ...
            if (this.isStatementToken(argsString)) {
                ...
                if (argsString.length === 1) {
                    ...
                    args.push(CSL.getObjectValue(model, argsString.replace(Symbols.MODEL_PREFIX, "")));
                }
            } else if (CSL.isStateValue(argsString)) {
                if (argsString.length === 1) {
                    args.push(state);
                } else {
                    args.push(CSL.getObjectValue(state, argsString.replace(Symbols.STATE_PREFIX, "")));
                }
    ...
    private getFilteredTokenArray(tokens: string[], tokenPrefix: string): string[] {
        return tokens.filter(token => token.startsWith(tokenPrefix))
