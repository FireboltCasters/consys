...
const interpretFromSource: (
  source: string,
  model?: any,
  state?: any,
  functions?: any
) => any = (source: string, model: any, state: any, functions: any) => {
...
test('Interpreter returns nothing with no valid ast', () => {
  const interpreter = new Interpreter();
  const ast = new Expression.AST(null, '', {counts: {model: {}, state: {}}});
...
test('Interpreter returns correct result for trivial expressions', () => {
  ...
  expect(
    interpretFromSource(
      'ALWAYS: ((1 + (2 * (3 + 4) + 5) - 6 + (7 + 8 + 1)) / 3.0) % 2'
    )
  ).toBe(0);
});
...
test('Interpreter returns correct result with model and state variables', () => {
  ...
  expect(
    interpretFromSource('ALWAYS: $value == #first / 3', model, state)
  ).toBe(true);
  expect(
    interpretFromSource(
      'ALWAYS: $nested.value == #second.third / 2',
      model,
      state
    )
  ).toBe(true);
  expect(interpretFromSource('ALWAYS: $', model, state)).toBe(
...
test('Interpreter returns correct result with function calls', () => {
  ...
  expect(
    interpretFromSource(`ALWAYS: LENGTH('Testing')`, null, null, functions)
  ).toBe(7);
  expect(
    interpretFromSource(`ALWAYS: LENGTH('Test ing')`, null, null, functions)
  ).toBe(8);
  expect(
    interpretFromSource(`ALWAYS: TRUE AND FALSE`, null, null, functions)
  ).toBe(false);
  expect(
    interpretFromSource(`ALWAYS: TRUE AND TRUE`, null, null, functions)
  ).toBe(true);
  expect(
    interpretFromSource(`ALWAYS: FALSE OR TRUE`, null, null, functions)
  ).toBe(true);
});
...
test('Interpreter throws error on invalid inputs', () => {
  const model = {value: 10, nested: {value: 20}};
  const state = {first: 30, second: {third: 40}};
  expect(interpretFromSource('ALWAYS: $val == #first / 3', model, state)).toBe(
    false
  );
  expect(
    interpretFromSource('ALWAYS: $value == #fourth / 3', model, state)
  ).toBe(false);
  expect(
    interpretFromSource('ALWAYS: $value == #second.third / 0', model, state)
  ).toBe(false);
  expect(
    interpretFromSource(
      'ALWAYS: $value == #second.third / (42 - 42)',
      model,
      state
    )
  ).toBe(false);
  ...
  expect(
    interpretFromSource('ALWAYS: FUN($value) == #second.third', model, state)
  ).toBe(false);
});
