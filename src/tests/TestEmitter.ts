...
test('Emitter returns empty string with no provided ast', () => {
    const emitter = new Emitter({root: null, source: "", statistics: {counts: {model: {}, state: {}}}});
...
test('Emitter replaces model and state identifiers with field', () => {
    ...
    ).toContain('if(true){return(this.model.first<this.state.second.third);}else{return(true);}');
});

test('Emitter replaces model and state with objects', () => {
    expect(
        emitFromSource('ALWAYS : $ != #')
    ).toContain('if(true){return(this.model!=this.state);}else{return(true);}');
});

test('Emitter replaces function expression with field', () => {
    expect(
        emitFromSource('ALWAYS : (FUNCTION)')
    ).toContain(`if(true){return((this.functions['FUNCTION'](this.model,this.state)));}else{return(true);}`);
});

test('Emitter calls function with correct arguments', () => {
    expect(
        emitFromSource(`ALWAYS : F(3.5, 'S', $, G(42))`)
    ).toContain(`if(true){return(this.functions['F'](3.5,'S',this.model,this.functions['G'](42)));}else{return(true);}`);
});

test('Emitter replaces AND, OR and NOT keywords with js equivalent', () => {
    expect(
        emitFromSource('ALWAYS : NOT A(-1) AND B(2) OR C(3)')
    ).toContain(`if(true){return(!this.functions['A'](-1)&&this.functions['B'](2)||this.functions['C'](3));}else{return(true);}`);
});
