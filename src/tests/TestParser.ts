...
const parseFromSource: (source: string) => Expression.AST = (source: string) => {
...
test('Parser does not throw error from valid syntax', () => {
    ...
    expect(!!parseFromSource('ALWAYS : A >= $B / #C OR D && E % F > G').root).toBe(true);
    expect(!!parseFromSource('ALWAYS : -A >= ($B / #C) OR D && NOT E % F > !G').root).toBe(true);
});
