...
export default class Constraint<T extends ConstraintData, M, S> {
    ...
    evaluate<D extends { model: M, state: S, functions: { [key: string]: Function } }>(data: D): Evaluation {
        let consistent = this.assertionFunction.apply(data, null);
        return {
            consistent: consistent,
            message: consistent ? "" : this.csl.getMessage(this.resource.message, data.model, data.state, data.functions),
