...
export default class EventDownloadHelper {
  ...
  static async downloadEventsForCities(year: number, cities: City[]){
    ...
    for(const city of cities){
      const cityEvents = await EventDownloadHelper.downloadEventsForCity(year, city);
  ...
  static async downloadEventsForCity(year: number, city: City){
    ...
    for(const street of streets){
      const streetEvents = await EventDownloadHelper.downloadEventsForStreet(year, city, street);
